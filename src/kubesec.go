...
var (
	...
	fileCreateFlags             = os.O_WRONLY | os.O_CREATE
	root            string
	overwrite       bool
)
...
func kubesec() {
	...
	if err := os.MkdirAll(root, publicPerms); err != nil {
	...
	switch subCommand {
	case "ca":
		createCA(flag.Args()[2:], "ca", "/CN=certstore-ca", time.Hour*24*(10*365+5))
	case "ica":
		createCA(flag.Args()[2:], "ca/ica", "/CN=certstore-ica", time.Hour*24*(5*365+5))
	case "server":
...
func createCA(args []string, path string, defaultDN string, defaultValidity time.Duration) {
	...
	if err := os.MkdirAll(path, publicPerms); err != nil {
